...
在 `minimal` 网络、`T` 场景（运输中断）、Qwen3-1.7B 下，仅 10 个 episode 的对比：

| 指标 | 纯 MAPPO（前10轮）| Hybrid LLM+MAPPO（前10轮）|
|------|------------------|---------------------------|
| 平均评分 | -18.09 | **-2.77** |
| CO2 封存量 | 11.16 Mt | **18.19 Mt** (+63%) |
| CO2 排放量 | 24.61 Mt | **17.71 Mt** (-28%) |
| 质量违规 | 9.3 次 | **6.5 次** |

LLM 的领域知识（识别中断、比较运输成本、判断纯度是否达标）开箱即用，无需任何训练。
